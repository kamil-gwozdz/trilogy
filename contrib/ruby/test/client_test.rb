...
class ClientTest < TrilogyTest
  ...
  def test_packet_size_lower_than_trilogy_max_packet_len
    ...
    exception = assert_raises Trilogy::QueryError do
        query_offset = "INSERT INTO trilogy_test (blob_test) VALUES ('')".bytesize
        result = client.query "INSERT INTO trilogy_test (blob_test) VALUES ('#{"x" * ((4 * 1024 * 1024) - query_offset)}')"
    end
  ...
  return
    ...
    result = client.query "INSERT INTO trilogy_test (blob_test) VALUES ('#{"x" * ((1023 * 1024 * 1024))}')"
    assert result
    assert_equal 1, client.last_insert_id

    exception = assert_raises Trilogy::QueryError do
        query_offset = "INSERT INTO trilogy_test (blob_test) VALUES ('')".bytesize
        result = client.query "INSERT INTO trilogy_test (blob_test) VALUES ('#{"x" * ((1024 * 1024 * 1024) - query_offset)}')"
    end
  ensure
    ensure_closed client
  end
  ...
  def test_packet_size_greater_than_trilogy_max_packet_len
    ...
    set_max_allowed_packet(x) # TRILOGY_MAX_PACKET_LEN is 16MB

    client = new_tcp_client(max_allowed_packet: x)
    ...
    query_offset = "INSERT INTO trilogy_test (blob_test) VALUES ('')".bytesize + 1 # +1 for the packet type byte
    result = client.query "INSERT INTO trilogy_test (blob_test) VALUES ('#{"x" * ((x) - query_offset - 3)}')"
    assert result
    assert_equal 3, client.last_insert_id

    exception = assert_raises Trilogy::QueryError do # -1 raises
        result = client.query "INSERT INTO trilogy_test (blob_test) VALUES ('#{"x" * ((x) - query_offset - 2)}')"
    end
